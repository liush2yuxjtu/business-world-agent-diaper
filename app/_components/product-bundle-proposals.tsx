...
export function ProductBundleProposals({ data, onSource }: { data: BusinessPayload | null; onSource?: () => void }) {
  ...
  return <section className="panel bundle-proposals" aria-label="组合商品研究提案">
    ...
    {associations && <><h3>关联购买依据</h3><p>{associations.source} · {associations.period} · 样本 {associations.sampleOrders.toLocaleString('zh-CN')} 单</p>
      <div className="table-scroll" role="region" tabIndex={0} aria-label="关联购买表格，支持横向滚动"><table><thead><tr><th scope="col">组合提案</th><th scope="col">同单数</th><th scope="col">样本内占比</th></tr></thead><tbody>{sourcedDrafts.map(item => <tr key={item.id} className="entity-row" onClick={event => { if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey || (event.target as HTMLElement).closest("a,button,input,select,textarea")) return; event.currentTarget.querySelector<HTMLButtonElement>("button:not([disabled])")?.click(); }}><td><button type="button" onClick={event => { trigger.current = event.currentTarget; setOpenId(item.id); }}>查看关联组合：{item.association!.row.title}</button></td><td>{item.association!.row.jointOrders}</td><td>{(item.association!.row.jointOrders / associations.sampleOrders * 100).toFixed(1)}%</td></tr>)}</tbody></table></div>
    ...
    <div className="table-scroll" role="region" tabIndex={0} aria-label="组合提案表格，支持横向滚动"><table aria-label="组合提案列表"><thead><tr><th scope="col">组合</th><th scope="col">研究假设</th><th scope="col">状态</th></tr></thead><tbody>{drafts.map(item => <tr key={item.id} className="entity-row" onClick={event => { if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey || (event.target as HTMLElement).closest("a,button,input,select,textarea")) return; event.currentTarget.querySelector<HTMLButtonElement>("button:not([disabled])")?.click(); }}><td><button type="button" onClick={event => { trigger.current = event.currentTarget; setOpenId(item.id); }}>查看组合提案 {item.id}</button></td><td>{item.hypothesis}</td><td>待核查 · 未执行</td></tr>)}</tbody></table></div>
