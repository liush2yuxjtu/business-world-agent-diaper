...
type JevAnswers = {
  ...
  risk: ScoreAnswer;
  human_approval: NoulAnswer;
  injection_signal: NoulAnswer;
};
...
function normalizeChoice(choice: string, entries: Array<[string, number]>): ChoiceAnswer {
...
function demoAnswers(state: string): JevAnswers {
...
export async function POST(request: NextRequest) {
  ...
  let body: { state?: unknown; apiKey?: unknown };
  try {
  ...
  if (!apiKey) {
    const answers = demoAnswers(state);
    return json({
      mode: 'demo',
  ...
  if (!upstream.ok) {
    const safeMessage =
      upstream.status === 401 || upstream.status === 403
        ? 'TypeSafe rejected the API key or this account does not have Jev API access yet.'
  ...
  const answers = raw?.answers as Partial<JevAnswers> | undefined;
  ...
  return json({
    ...
    estimatedInputCostUsd: inputTokens != null ? inputTokens * 0.042 / 1_000_000 : null,
