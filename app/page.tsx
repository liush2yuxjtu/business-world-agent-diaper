...
function EmptyState({ title, onConnect }: { title: string; onConnect: () => void }) {
  return <section className="panel empty-panel">
    ...
    <p>当前没有经过验证的业务快照。产品不会用 Demo、fixture 或硬编码数字替代真实数据。</p>
...
function SourceBanner({ snapshot, onEdit }: { snapshot: Snapshot | null; onEdit: () => void }) {
  ...
  return <div className={`source-banner ${real ? 'real' : 'missing'}`}>
    <div><ShieldCheck size={18}/><span><b>{real ? 'VERIFIED PERSISTED SOURCE' : 'NO VERIFIED SOURCE'}</b>{real ? `${snapshot?.provenance.sourceLabel} · ${snapshot?.provenance.provider}` : '生产路径不会回退到 mock'}</span></div>
...
function Overview({ data, onConnect }: { data: Payload | null; onConnect: () => void }) {
  ...
  return <>
    <section className="hero-panel"><div><div className="eyebrow">真实经营状态</div><h2>同一份持久化数据同时服务 UI 与 Agent</h2><p>这里只呈现已写入系统记录的数据；未知值保持为空。</p></div><div className="hero-baby">REAL DATA<br/><small>NO SILENT MOCK FALLBACK</small></div></section>
...
function DataView({ title, description, metrics, data, onConnect }: { title: string; description: string; metrics: Array<[string, string]>; data: Payload | null; onConnect: () => void }) {
  ...
  return <section className="panel data-surface"><div className="section-title"><span>{title}</span><small>Persisted observation</small></div><p>{description}</p><div className="data-grid">{metrics.map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}</div></section>;
}
...
function ReportView({ snapshot, onConnect }: { snapshot: Snapshot | null; onConnect: () => void }) {
  ...
  return <section className="panel report"><div className="section-title"><span>Evidence Report</span><small>可追溯</small></div><dl><div><dt>Source</dt><dd>{snapshot.provenance.sourceLabel}</dd></div><div><dt>Provider</dt><dd>{snapshot.provenance.provider}</dd></div><div><dt>Observed at</dt><dd>{snapshot.provenance.asOf}</dd></div><div><dt>Storage</dt><dd>{snapshot.provenance.storage}</dd></div><div><dt>Last persisted</dt><dd>{snapshot.provenance.updatedAt}</dd></div></dl><pre>{JSON.stringify(snapshot.data, null, 2)}</pre></section>;
}
...
function ExperimentView({ snapshot }: { snapshot: Snapshot | null }) {
  ...
  return <section className="panel experiment"><div className="section-title"><span>Scenario Experiment</span><small>MODELLED · NOT OBSERVED</small></div><p>使用当前真实快照作为 baseline。模型结果明确标注为数学推演，不再伪装成 AI 已预测的市场事实。</p><textarea value={prompt} onChange={e => setPrompt(e.target.value)}/><div className="experiment-controls"><select value={lever} onChange={e => setLever(e.target.value)}><option value="ad_efficiency">投放效率</option><option value="content_engagement">内容互动</option><option value="live_watch_time">直播观看</option><option value="checkout_conversion">交易转化</option><option value="repeat_purchase">复购</option></select><input type="number" value={changePercent} min={-80} max={200} onChange={e => setChangePercent(e.target.value)}/><button className="primary" onClick={run} disabled={!snapshot?.data}><Play size={16}/>运行并保存</button></div>{!snapshot?.data && <p className="warning">先连接真实 baseline，系统不会对空数据制造预测。</p>}{status && <p className="status-line">{status}</p>}{result && <pre>{JSON.stringify(result, null, 2)}</pre>}</section>;
}
...
function DataEditor({ snapshot, onSaved, onClose, readOnly }: { snapshot: Snapshot | null; onSaved: () => void; onClose: () => void; readOnly: boolean }) {
  ...
  async function submit(event: FormEvent) {
    event.preventDefault(); if (readOnly) { setStatus('当前 verified Supabase baseline 为只读；平台写入凭据接入后才允许修改。'); return; } setStatus('保存中…');
    ...
    try {
      ...
      setStatus('已写入 Neon Postgres'); await onSaved();
  ...
  return <div className="editor-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}><form className="data-editor" onSubmit={submit}><div className="editor-head"><div><b>{readOnly ? 'Verified Business World Source' : '连接真实 Business World 数据'}</b><span>{readOnly ? '当前读取 Supabase 持久化基线；平台指标保持未观测，不用 mock 补值。' : '保存后 UI 与 Eve tools 读取同一持久化记录。'}</span></div><button type="button" onClick={onClose}>×</button></div><label>来源名称<input required disabled={readOnly} value={sourceLabel} onChange={e => setSourceLabel(e.target.value)}/></label><label>观测时间<input required disabled={readOnly} type="datetime-local" value={observedAt} onChange={e => setObservedAt(e.target.value)}/></label><div className="field-grid">{fields.map(([key,label,unit]) => <label key={key}>{label}<div className="unit-input"><input type="number" step="any" disabled={readOnly} value={values[key]} onChange={e => set(key,e.target.value)}/><span>{unit}</span></div></label>)}</div><label>来源说明 / Evidence<textarea disabled={readOnly} value={values.notes} onChange={e => set('notes',e.target.value)} placeholder="例如：来自 2026-09-17 店铺后台导出；文件已由运营核验。"/></label><div className="editor-actions"><span>{status}</span><button className="primary" type="submit" disabled={readOnly}><Save size={16}/>{readOnly ? '只读来源' : '保存真实快照'}</button></div></form></div>;
}
