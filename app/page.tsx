...
function SourceBanner({ snapshot, onEdit }: { snapshot: Snapshot | null; onEdit: () => void }) {
  ...
  return <div className={`source-banner ${real ? 'real' : 'missing'}`}>
    ...
    <button onClick={onEdit}>{real ? '更新来源' : '连接数据'}</button>
  </div>;
}
...
function DataEditor({ snapshot, onSaved, onClose }: { snapshot: Snapshot | null; onSaved: () => void; onClose: () => void }) {
  ...
  return <div className="editor-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}><form className="data-editor" onSubmit={submit}><div className="editor-head"><div><b>连接真实 Business World 数据</b><span>保存后 UI 与 Eve tools 读取同一持久化记录。</span></div><button type="button" onClick={onClose}>×</button></div><label>来源名称<input required value={sourceLabel} onChange={e => setSourceLabel(e.target.value)}/></label><label>观测时间<input required type="datetime-local" value={observedAt} onChange={e => setObservedAt(e.target.value)}/></label><div className="field-grid">{fields.map(([key,label,unit]) => <label key={key}>{label}<div className="unit-input"><input type="number" step="any" value={values[key]} onChange={e => set(key,e.target.value)}/><span>{unit}</span></div></label>)}</div><label>来源说明 / Evidence<textarea value={values.notes} onChange={e => set('notes',e.target.value)} placeholder="例如：来自 2026-09-17 店铺后台导出；文件已由运营核验。"/></label><div className="editor-actions"><span>{status}</span><button className="primary" type="submit"><Save size={16}/>保存真实快照</button></div></form></div>;
}
